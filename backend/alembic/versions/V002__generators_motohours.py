...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("initial_motohours", sa.Numeric(10, 2), nullable=True, server_default="0"),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column(
            "updated_by",
            UUID(as_uuid=True),
    ...
    op.create_table(
        ...
        sa.Column("next_service_at_hours", sa.Numeric(10, 2), nullable=False),
    ...
    op.create_index("ix_motohours_log_generator_id", "motohours_log", ["generator_id"])
    op.create_index("ix_maintenance_log_generator_id", "maintenance_log", ["generator_id"])
    op.create_index("ix_event_log_generator_id", "event_log", ["generator_id"])
    op.create_index("ix_event_log_event_type", "event_log", ["event_type"])


def downgrade() -> None:
    op.drop_index("ix_event_log_event_type", table_name="event_log")
    op.drop_index("ix_event_log_generator_id", table_name="event_log")
    op.drop_index("ix_maintenance_log_generator_id", table_name="maintenance_log")
    op.drop_index("ix_motohours_log_generator_id", table_name="motohours_log")
