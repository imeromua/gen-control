...
class MotohoursService:
    ...
    async def get_log(self, generator_id: uuid.UUID) -> list[MotohoursLogResponse]:
    ...
    async def get_total(self, generator_id: uuid.UUID) -> MotohoursTotalResponse:
        ...
        return MotohoursTotalResponse(generator_id=generator_id, motohours_total=total)

    async def get_maintenance_log(self, generator_id: uuid.UUID) -> list[MaintenanceLogResponse]:
        await self._ensure_generator_exists(generator_id)
    ...
    async def create_maintenance(
        self, generator_id: uuid.UUID, data: MaintenanceCreate, current_user_id: uuid.UUID
        ...
        to_interval = (
            Decimal(str(settings.to_interval_hours)) if settings and settings.to_interval_hours else None
        )
        next_service_at_hours = motohours_total + to_interval if to_interval is not None else None
        ...
        event = EventLog(
            ...
            meta={
                ...
                "next_service_at_hours": str(next_service_at_hours) if next_service_at_hours else None,
            },
        )
